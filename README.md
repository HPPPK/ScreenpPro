...
- **画布编辑器**：添加、选中、拖动、调整文字、图片、时钟组件。支持图层上下移动/置顶/置底、复制、多选、删除、方向键微调、Shift+方向键快速移动、对齐线、网格吸附、撤销/重做、画布缩放、组件锁定/隐藏和自定义组件名称；右键组件或图层可快速复制、删除、调整层级、锁定/隐藏，并恢复该组件的属性默认值。
- **扩展组件库**：日期、倒计时、进度条、世界时钟、二维码、网页预览、GitHub 仓库动态、RSS、句子轮播、照片墙、天气、电池、系统概览和网络状态；新增月历、专注计时、日进度和信息卡片四类离线组件。
- **网页预览限制**：网页组件仅允许 http:// 与 https://，使用本机 Edge/Chrome 生成整页缩略图；运行期间复用无头浏览器进程并缓存相同 URL/尺寸结果，浏览器不可用或网络失败时显示明确错误，不会让整个屏保白屏。网页、GitHub、RSS、天气等网络组件仅在运行时刷新，编辑器预览会减少轮询。
...
2. 在编辑器添加文字、图片、时钟，并拖动组件完成布局；
   编辑器左侧可以添加所有首发组件；选中组件后，在右侧属性面板配置内容。属性面板的“恢复属性默认值”只重置当前组件的内容属性，不会改变位置、尺寸、名称、图层或锁定状态。网页、GitHub、RSS、天气需要网络，网络失败时只显示组件错误状态，不会让整个屏保白屏。
3. 在 **我的库** 将作品设为“当前屏保”；
...
Windows 发布由 GitHub Actions 自动完成。推送形如 `v0.1.8` 的 tag 后，会构建、签名并发布 Windows NSIS 安装包以及更新清单。
...
多显示器、DPI、任务视图和窗口切换的现场测试步骤见 [TESTING.md](TESTING.md)。当前开发机只有一个 1920 × 1080 显示器，因此不会把多屏验证标记为已通过。
