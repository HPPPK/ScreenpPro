...
**ScreenPro** 是面向 Windows 和 macOS 的可自定义桌面屏幕保护应用。它以覆盖式全屏窗口保护屏幕，同时不终止下载、渲染、同步或其他后台程序。

> ScreenPro 是应用级屏幕保护，不是 Windows/macOS 系统锁屏。系统安全快捷键、强制退出和系统账户安全仍由操作系统控制。
...
- **密码退出**：使用本机应用独立密码解锁；凭据采用 Argon2id 哈希保存，支持安全问题重设。
